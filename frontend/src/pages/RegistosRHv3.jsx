...
const ToolbarFilters = ({ dataAPI, auth, num, v, ...props }) => {
  return (<>
    {isRH(auth, num) && <>
      <Col width={80}>
        <Field name="fnum" shouldUpdate label={{ enabled: true, text: "Número", pos: "top", padding: "0px" }}>
          <Input size='small' allowClear />
        </Field>
      </Col>
      <Col width={180}>
        <Field name="fnome" shouldUpdate label={{ enabled: true, text: "Nome", pos: "top", padding: "0px" }}>
          <Input size='small' allowClear />
        </Field>
      </Col>
    </>}
    <Col xs='content'>
      <Field name="fdata" shouldUpdate label={{ enabled: true, text: "Data", pos: "top", padding: "0px" }}>
...
const RegistosVisuaisViewer = ({ p, column, title, forInput, ...props }) => {
  ...
  const loadData = async ({ signal } = {}) => {
    try {
      let response = await fetchPost({ url: `${API_URL}/rponto/sqlp/`, withCredentials: true, filter: {}, parameters: { method: "GetCameraRecords", date: dayjs(p.data.dts).format(DATE_FORMAT_NO_SEPARATOR), num: p.data.num } });
      if (response.data.status !== "error") {
        setRecords(response.data);
      } else {
        setRecords([]);
...
export default ({ setFormTitle, ...props }) => {
  ...
  const dataAPI = useDataAPI({ id: props.id, payload: { url: `${API_URL}/rponto/sqlp/`, withCredentials: true, parameters: defaultParameters, pagination: { enabled: true, page: 1, pageSize: 20 }, filter: defaultFilters } });
  const submitting = useSubmitting(true);
  ...
  const editableClass = (cellProps, { data }, ty) => {
  ...
  const columns = [
    ...isRH(auth, num) ? [{ name: 'num', header: 'N', userSelect: true, defaultLocked: true, frozen: true, width: 70, render: (p) => <div style={{ fontWeight: 900 }}>{p.data?.num}</div> }] : [],
  ...
  const loadData = async ({ init = false, signal } = {}) => {
    if (init) {
      const { num: _num, tstamp, ...initFilters } = loadInit({}, { ...dataAPI.getAllFilter(), tstamp: dataAPI.getTimeStamp() }, props, { ...location?.state }, [...Object.keys(location?.state ? location?.state : {}), ...Object.keys(dataAPI.getAllFilter())]);
      console.log("init-load->", dataAPI.getFilter(), dataAPI.getSort())
      setNum(_num);
      let { filterValues, fieldValues } = fixRangeDates(['fdata'], initFilters);
      console.log("4....")
      formFilter.setFieldsValue({ ...fieldValues });
      dataAPI.addFilters({ ...filterValues, ...(_num && { num: _num }) }, true);
  ...
  return (
    <>
      ...
      <Table
        ...
        toolbarFilters={{
          form: formFilter, schema, onFinish: onFilterFinish, onValuesChange: onFilterChange,
          filters: <ToolbarFilters dataAPI={dataAPI} auth={auth} num={num} v={formFilter.getFieldsValue(true)} />,
          moreFilters: { schema: isRH(auth, num) ? moreFiltersSchema : moreFiltersSchemaPrivate }
        }}
      />
    </>
  );
