...
def processRecord(num, ts):
    ...
    """
    try:
        rows = dbsage100c.executeSimpleList(lambda: sql, connection, {})["rows"]
        # Para manter compatibilidade com outros métodos:
        return {"date_ref": ts.strftime("%Y-%m-%d"), "funcionarios": rows}
    except Exception as error:
        print(str(error))
        return {"date_ref": ts.strftime("%Y-%m-%d"), "funcionarios": []}
